...
This section provides a high-level overview and installation procedure for Istio's ambient mode on OpenShift Container Platform (OCP) using OpenShift Service Mesh 3.
...
**Coexistence with Sidecar-based OSSM 3:**

While it is technically possible for a sidecar version of OSSM to exist on the same cluster, for a seamless tech preview experience and to avoid potential conflicts, it is **highly recommended** to use a clean OpenShift cluster without any pre-existing OSSM installations (especially 2.6 or earlier) when setting up ambient mode. This ensures a dedicated environment for testing the new architecture.

---

## 3. Procedure to Install Istio's Ambient Mode

This procedure demonstrates how to install Istio's ambient mode on OpenShift and integrate the Bookinfo application.

### 3.1 Prerequisites

* You have deployed a cluster on OpenShift Container Platform 4.19 or later.
* You are logged in to the OpenShift Container Platform we console as a user with the cluster-admin role.

### 3.2 Install Service Mesh 3 Operator

The `sail-operator` manages the deployment and configuration of Istio ambient mode components.

Follow the official docs steps to deploy [Service Mesh Operator](https://docs.redhat.com/en/documentation/red_hat_openshift_service_mesh/3.0/html/installing/ossm-installing-service-mesh#ossm-installing-operator_ossm-about-deploying-istio-using-service-mesh-operator)

### 3.3 Install Istio (Control Plane)

#### 3.3.1 Creating the Istio project using the web console

**Prerequisites**

1.  The Red Hat OpenShift Service Mesh Operator must be installed.
2.  You are logged in to the OpenShift Container Platform web console as cluster-admin.
...
#### 3.3.2 Creating the Istio resource using the web console
...
#### 3.3.3 Creating the Istio resource using the CLI

TBD

### 3.4 Install Istio CNI

#### 3.4.1 Creating the Istio CNI project using the web console
...
#### 3.4.2 Creating the Istio CNI resource using the web console
...
10. Click **Create**. This action deploys the Istio control plane.  
    When `State: Healthy` appears in the **Status** column, Istio is successfully deployed.

#### 3.4.3 Creating the Istio CNI resource using the CLI

TBD

### 3.5 Install Ztunnel proxy

#### 3.5.1 Creating the Ztunnel project using the web console
...
3.  At the prompt, enter a name for the project in the **Name** field. For example, `ztunnel`. The other fields provide supplementary information to the `Istio` resource definition and are optional.  
    **Note** - The namespace name for `ztunnel` project have to equal the `trustedZtunnelNamespace` parameter in **Istio** configuration.
4.  Click **Create**. The Service Mesh Operator deploys Istio to the project you specified.

#### 3.5.2 Creating the Istio CNI resource using the web console
...
#### 3.5.3 Creating the Istio CNI resource using the CLI

TBD

### 3.6 Scoping the Service Mesh with discovery selectors
...
#### 3.6.1 About discovery selectors
...
#### 3.6.2 Scoping a Service Mesh by using discovery selectors
...
2. Add a label to the namespace containing the Ztunnel proxy, for example, the `ztunnel` namespace.
...
### 3.7 About the Bookinfo Application
...
#### 3.7.1 Deploying the Bookinfo application
...
1. Create the `bookinfo` namespace and add a label `istio-descovery=enabled`.
